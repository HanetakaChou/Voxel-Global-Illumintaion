...
{
    // [unroll] 
    for (int clipmap_level_index = 0; clipmap_level_index < int(CLIPMAP_LEVEL_COUNT); ++clipmap_level_index)
    {
        ...
        float3 ray_direction_texel_space;
        {
            ...
            // texel space: 0 - 127
            float clipmap_level_voxel_size = float(FINEST_VOXEL_SIZE) * float(1 << clipmap_level_index);

            ray_origin_texel_space = (ray_origin_world_space - g_clipmap_center) * (1.0 / clipmap_level_voxel_size) + float(CLIPMAP_SIZE) * 0.5;
        ...
        float t_max;
        {
            ...
            float3 box_max_texel_space = float3(float(CLIPMAP_SIZE) - margin, float(CLIPMAP_SIZE) - margin, float(CLIPMAP_SIZE) - margin);
        ...
        float t_delta;
        {
            ...
            [branch] 
            if (abs_ray_direction_texel_space.x > abs_ray_direction_texel_space.y && abs_ray_direction_texel_space.x > abs_ray_direction_texel_space.z)
            {
        ...
        //[loop] 
        for (int unused_index = 0; unused_index < 128; ++unused_index)
        {
            [branch] 
            if (t_current <= t_min)
            {
            ...
            [branch] 
            if (any(voxel_clipmap_texel_space < int3(0, 0, 0)) || any(voxel_clipmap_texel_space >= int3(CLIPMAP_SIZE, CLIPMAP_SIZE, CLIPMAP_SIZE)))
            {
                continue;
            }

            int3 voxel_clipmap_toroidal_address = calculate_clipmap_toroidal_address(voxel_clipmap_texel_space, clipmap_level_index);

            uint packed_opacity;
#if 5 == CLIPMAP_LEVEL_COUNT
            [branch] 
            if (0 == clipmap_level_index)
            {
                packed_opacity = g_clipmap_opacity[0].Load(int4(voxel_clipmap_toroidal_address, 0));
            }
            else if (1 == clipmap_level_index)
            {
                packed_opacity = g_clipmap_opacity[1].Load(int4(voxel_clipmap_toroidal_address, 0));
            }
            else if (2 == clipmap_level_index)
            {
                packed_opacity = g_clipmap_opacity[2].Load(int4(voxel_clipmap_toroidal_address, 0));
            }
            else if (3 == clipmap_level_index)
            {
                packed_opacity = g_clipmap_opacity[3].Load(int4(voxel_clipmap_toroidal_address, 0));
            }
            else if (4 == clipmap_level_index)
            {
                packed_opacity = g_clipmap_opacity[4].Load(int4(voxel_clipmap_toroidal_address, 0));
            }
            else
            {
                // Error
                // 5 == CLIPMAP_LEVEL_COUNT
                packed_opacity = 0;
...
#error TODO
            ...
            if (packed_opacity != 0)
            {
                float opacity = unpack_opacity(packed_opacity);

                float3 opacity_color = lerp(float3(0.0, 0.0, 1.0), float3(1.0, 0.0, 0.0), opacity);
