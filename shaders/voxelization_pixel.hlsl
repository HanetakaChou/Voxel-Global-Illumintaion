...
#include "config.sli"
#include "consts.sli"
#include "voxelization_shader_resource_table.sli"
...
void main(in uint in_sample_mask : SV_Coverage, in float4 in_position : SV_Position, in float in_cull_distance[2] : SV_CullDistance, in nointerpolation int in_viewport_depth_direction_index : TEXCOORD0, in nointerpolation int in_clipmap_level_index : TEXCOORD1)
{
	...
	// TODO: why flip y ???
	float pixel_clipmap_texel_space_viewport_y_direction_component = float(CLIPMAP_SIZE) - in_position.y;
	float pixel_clipmap_texel_space_viewport_depth_direction_component = in_position.z * float(CLIPMAP_SIZE);
...
#if 8 == VOXELIZATION_SAMPLE_COUNT
	...
	const float2 voxelization_sample_positions[VOXELIZATION_SAMPLE_COUNT] = {
...
#error TODO
	...
	float opacity_sample_scale[voxel_count];
	{
		...
		[unroll]
		for (int voxelization_sample_index = 0; voxelization_sample_index < int(VOXELIZATION_SAMPLE_COUNT); ++voxelization_sample_index)
		{
			[branch]
			if (0 != (voxelization_sample_mask & (1 << voxelization_sample_index)))
			{
				...
				opacity_sample_scale[0] += (1.0 / float(VOXELIZATION_SAMPLE_COUNT)) * saturate(1.0 - abs(fractional_voxelization_sample_clipmap_texel_space_viewport_depth_direction_component - (-0.5)));
				opacity_sample_scale[1] += (1.0 / float(VOXELIZATION_SAMPLE_COUNT)) * saturate(1.0 - abs(fractional_voxelization_sample_clipmap_texel_space_viewport_depth_direction_component - 0.5));
				opacity_sample_scale[2] += (1.0 / float(VOXELIZATION_SAMPLE_COUNT)) * saturate(1.0 - abs(fractional_voxelization_sample_clipmap_texel_space_viewport_depth_direction_component - 1.5));
			}
		}
	}
