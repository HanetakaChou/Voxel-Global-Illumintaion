	...
	in uint in_vertex_id : SV_VertexID, in uint in_instance_id : SV_InstanceID, out float4 out_position : SV_Position, out float out_cull_distance[2] : SV_CullDistance, out nointerpolation int out_viewport_depth_direction_index : TEXCOORD0, out nointerpolation int out_clipmap_level_index : TEXCOORD1)
{
	...
	// For debug
	// 0 - 2
	int viewport_depth_direction_index = in_instance_id % int(VIEWPORT_DEPTH_DIRECTION_COUNT);
	// assert(clipmap_level_index < VIEWPORT_DEPTH_DIRECTION_COUNT);

	// 0 - 4
	int clipmap_level_index = in_instance_id / int(VIEWPORT_DEPTH_DIRECTION_COUNT);
	// assert(clipmap_level_index < CLIPMAP_LEVEL_COUNT);
	...
	cull_distance[0] = position_clip_space.z - (-1.0 / (float(CLIPMAP_SIZE) * 0.5));
	cull_distance[1] = (1.0 + 1.0 / (float(CLIPMAP_SIZE) * 0.5)) - position_clip_space.z;
