...
#define LIGHT_INJECTION_SAMPLE_COUNT 8

#define LIGHT_INJECTION_VIEWPORT_DEPTH_DIRECTION_COUNT 3
#define LIGHT_INJECTION_VIEWPORT_DEPTH_DIRECTION_AXIS_Z 0 // xyz -> xyz
#define LIGHT_INJECTION_VIEWPORT_DEPTH_DIRECTION_AXIS_Y 1 // xyz -> zxy
#define LIGHT_INJECTION_VIEWPORT_DEPTH_DIRECTION_AXIS_X 2 // xyz -> yzx

#if defined(__fsl)

CBUFFER(light_injection_render_pass_set_per_frame_rootcbv, UPDATE_FREQ_NONE, b0, binding = 0)
{
	DATA(float4x4, g_view_transform, None);
	DATA(float4x4, g_projection_transform, None);
};

CBUFFER(light_injection_render_pass_set_per_draw_rootcbv, UPDATE_FREQ_NONE, b1, binding = 1)
{
	DATA(float4x4, g_model_transform, None);
};

CBUFFER(light_injection_mesh_material_set_constant_buffer, UPDATE_FREQ_PER_DRAW, b0, binding = 0)
{
