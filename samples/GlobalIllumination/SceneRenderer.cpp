...
SceneRenderer::SceneRenderer(NVRHI::IRendererInterface *pRenderer)
    : m_RendererInterface(pRenderer), m_pScene(NULL), m_pInputLayout(NULL), m_pDefaultVS(NULL), m_pFullScreenQuadVS(NULL), m_pAttributesPS(NULL), m_pBlitPS(NULL), m_pCompositingPS(NULL), m_pGlobalCBuffer(NULL), m_pDefaultSamplerState(NULL), m_pComparisonSamplerState(NULL), m_Width(0), m_Height(0), m_SampleCount(1), m_pVoxelizationGS(NULL), m_pVoxelizationPS(NULL), m_TargetAlbedo(NULL), m_TargetNormal(NULL), m_TargetDepth(NULL), m_ShadowMap(NULL), m_NullTexture(NULL)
{
}
...
HRESULT SceneRenderer::AllocateResources(VXGI::IGlobalIllumination *pGI, VXGI::IShaderCompiler *pCompiler)
{
    ...
    // Voxelization GS
    {
...
#if USE_GL4
        {
            ...
            strcpy_s(desc.pixelShaderInputs[0].name, "v_texCoord");
            desc.pixelShaderInputs[0].width = 2;
            strcpy_s(desc.pixelShaderInputs[1].name, "v_normal");
            desc.pixelShaderInputs[1].width = 3;
            strcpy_s(desc.pixelShaderInputs[2].name, "v_tangent");
            desc.pixelShaderInputs[2].width = 3;
            strcpy_s(desc.pixelShaderInputs[3].name, "v_binormal");
            desc.pixelShaderInputs[3].width = 3;
            strcpy_s(desc.pixelShaderInputs[4].name, "v_positionWS");
            desc.pixelShaderInputs[4].width = 3;
...
void SceneRenderer::RenderShadowMap(const VXGI::Vector3f& lightPos, const VXGI::Matrix4f& lightViewMatrix, const VXGI::Matrix4f& lightProjMatrix)
{
    m_LightPos = lightPos;
    ...
    bool emittance)
{
    ...
    globalConstants.lightPos = (VXGI::Vector3f &)m_LightPos;
