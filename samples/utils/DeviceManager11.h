...
struct DeviceCreationParameters
{
    ...
    DeviceCreationParameters()
        : startMaximized(false), startFullscreen(false), backBufferWidth(1280), backBufferHeight(720), refreshRate(0), swapChainBufferCount(1), swapChainFormat(DXGI_FORMAT_R8G8B8A8_UNORM), swapChainUsage(DXGI_USAGE_SHADER_INPUT | DXGI_USAGE_RENDER_TARGET_OUTPUT), swapChainSampleCount(1), swapChainSampleQuality(0), driverType(D3D_DRIVER_TYPE_HARDWARE), featureLevel(D3D_FEATURE_LEVEL_11_0), enableDebugRuntime(false), adapterNameSubstring(L"")
    {
    }
};
...
public:
    DeviceManager()
        : m_Device(NULL), m_ImmediateContext(NULL), m_SwapChain(NULL), m_BackBufferRTV(NULL), m_IsNvidia(false), m_hWnd(NULL), m_WindowTitle(L""), m_FixedFrameInterval(-1), m_SyncInterval(0), m_AverageFrameTime(0), m_AverageTimeUpdateInterval(0.5), m_InSizingModalLoop(false), m_ShutdownCalled(false)
    {
    }
