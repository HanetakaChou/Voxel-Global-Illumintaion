...
void VxgiSampleVoxelData(float3 curPosition, float fLevel, float3 direction, float weight, bool flipOpacityDirections, out float opacity, out float3 emittance, out bool anyEmittance)
{
    ...
    opacity = VxgiSampleOpacityTextures(opacityCoords1.xyz, direction, flipOpacityDirections, sampleEmittance1) * weightLow + VxgiSampleOpacityTextures(opacityCoords2.xyz, direction, flipOpacityDirections, sampleEmittance2) * weightHigh;
...
VxgiConeTracingResults VxgiTraceCone(VxgiConeTracingArguments args)
{
    ...
    float coneAmbient = 0.0;

    // K(0) = -1
    float prevAmbientFactor = -1.0;
    ...
    gfsdk_Loop for (int sampleIndex = 0; sampleIndex < int(VCT_CONE_TRACING_SAMPLE_COUNT); ++sampleIndex)
    {
        ...
        float ambientFactor = K(args.ambientAttenuationFactor, t);
        coneAmbient += (ambientFactor - prevAmbientFactor) * transparency;
        ...
        cone_radiance += pTransparency * alpha * emittance;
    ...
    // k(inf) = 0
    coneAmbient += (0.0 - K(args.ambientAttenuationFactor, t)) * transparency;
...
float normalized_clamped_cosine(float3 omega_i)
{
    // PBR Book V3: [13.6.3 Cosine-Weighted Hemisphere Sampling](https://www.pbr-book.org/3ed-2018/Monte_Carlo_Integration/2D_Sampling_with_Multidimensional_Transformations#Cosine-WeightedHemisphereSampling)
    ...
    float cos_theta = max(0.0, omega_i.z);

    float pdf = cos_theta * (1.0 / M_PI);

    return pdf;
}

#define VCT_DIFFUSE_CONE_TRACING_CONE_COUNT 8

#define USE_DIFFUSE_SS_CORRECTION 0
#define SINGLE_DIFFUSE_OUTPUT (EMITTANCE_FORMAT == NONE || !SPARSE_TRACING)
TEXTURE2DARRAY(t_ConeDirectionMap, 10);
TEXTURECUBE(t_EnvironmentMap, 11);
SAMPLER(s_EnvironmentMapSampler, 11)
TEXTURE2D(t_ScreenSpaceOcclusion, 18);
#if defined(GFSDK_VXGI_GL)
...
#else
// t float4 gl_FragData[];
#define colorX gfsdk_ColorOutputX
layout(location = 0) out float4 gfsdk_ColorOutputX;
// efine colorX gl_FragData[0]
#define colorY gfsdk_ColorOutputY
layout(location = 1) out float4 gfsdk_ColorOutputY;
// efine colorY gl_FragData[1]
#define colorZ gfsdk_ColorOutputZ
layout(location = 2) out float4 gfsdk_ColorOutputZ;
// efine colorZ gl_FragData[2]
#endif
...
#endif
{
    ...
    if (all(gfsdk_GreaterThan(g_DownsampleScale.xy, float2(1.0, 1.0))))
    ...
    if (!IsInvalidNormal(sampleSmoothNormal) && confidence != 0)
    {
        ...
        {
            float3 omega_i = normalized_clamped_cosine_sample_omega_i(hammersley_2d(coneIndex, VCT_DIFFUSE_CONE_TRACING_CONE_COUNT));

            float3 rayDir = normalize(omega_i.x * tangent + omega_i.y * cotangent + omega_i.z * sampleSmoothNormal);

            float pdf = normalized_clamped_cosine(omega_i);
            ...
            float4 offsetParams = float4(g_InitialOffsetBias, g_InitialOffsetDistanceFactor, g_NormalOffsetFactor, minSampleSize * 0.5 * g_TracingStep);
...
#endif
            bool saveSamples = all(gfsdk_Equal(floor(gl_FragCoord.xy), floor(g_PixelToSave.xy * g_DownsampleScale.zw)));
            VxgiConeTracingArguments args = VxgiDefaultConeTracingArguments();
            ...
            //     radiosity.rgb += envMapScale * SampleTexLod(s_EnvironmentMapSampler, t_EnvironmentMap, gfsdk_CubemapSamplePos(rayDir), envMip).rgb * g_rNumCones;
            // }
            // monte carlo estimate: 1/N * 1/PI * albedo * L_i * cos_theta_i / (1/PI * cos_theta_i) = albedo * L_i
            // float3 radiance = (1.0 / float(VCT_DIFFUSE_CONE_TRACING_CONE_COUNT)) * cone.radiance;
            float3 radiance = cone.radiance;
#endif
    ...
    {
        color.rgb *= gfsdk_TextureLoad(t_ScreenSpaceOcclusion, int2(gl_FragCoord.xy)).rrr;
    }
    if (any(gfsdk_NotEqual(g_BackgroundColor.rgb, float3scalar(0))))
    {
