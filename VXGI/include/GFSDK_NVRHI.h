...
namespace NVRHI
{
    ...
    struct TextureDesc
    {
        ...
        TextureDesc() : format(Format::UNKNOWN),
                        width(0),
                        height(0),
                        depthOrArraySize(0),
                        mipLevels(1),
    ...
    struct ShaderDesc
    {
        ...
        ShaderDesc(ShaderType::Enum type)
            : shaderType(type), preCreationCommand(0), postCreationCommand(0), metadataValid(false), numPipelineStateExtensions(0)
        {
        }
    };
    ...
    struct BlendState
    {
        ...
        BlendState() : blendFactor(0, 0, 0, 0),
    ...
    struct DepthStencilState
    {
        ...
        DepthStencilState() : stencilRefValue(0),
                              depthEnable(true),
    ...
    struct PipelineStageBindings
    {
        ...
        PipelineStageBindings()
            : stage(ShaderType::SHADER_PIXEL), textureBindingCount(0), textureSamplerBindingCount(0), bufferBindingCount(0), constantBufferBindingCount(0), shader(nullptr), userDefinedShaderPermutationIndex(0)
        {
        ...
        PipelineStageBindings(ShaderType::Enum _stage)
            : stage(_stage), textureBindingCount(0), textureSamplerBindingCount(0), bufferBindingCount(0), constantBufferBindingCount(0), shader(nullptr), userDefinedShaderPermutationIndex(0)
        {
            memset(textures, 0, sizeof(textures));
            memset(textureSamplers, 0, sizeof(textureSamplers));
            memset(buffers, 0, sizeof(buffers));
    ...
    struct DrawCallState
    {
        ...
        DrawCallState()
            : VS(ShaderType::SHADER_VERTEX), HS(ShaderType::SHADER_HULL), DS(ShaderType::SHADER_DOMAIN), GS(ShaderType::SHADER_GEOMETRY), PS(ShaderType::SHADER_PIXEL), primType(PrimitiveType::TRIANGLE_LIST), inputLayout(nullptr), indexBuffer(nullptr), indexBufferFormat(Format::R32_UINT), indexBufferOffset(0), vertexBufferCount(0)
        {
    ...
    struct DrawArguments
    {
        ...
        DrawArguments()
            : vertexCount(0), instanceCount(1), startIndexLocation(0), startVertexLocation(0), startInstanceLocation(0)
        {
        }
    };
