...
namespace VXGI
{
    ...
    struct CommonTracingParameters
    {
        ...
        CommonTracingParameters()
            : maxSamples(128), tracingStep(1.0f), opacityCorrectionFactor(1.0f), irradianceScale(1.0f), flipOpacityDirections(false), debugParameters(0.0f), nearClipZ(0.0f), farClipZ(1.0f)
        {
        }
    };
    ...
    struct DiffuseTracingParameters : public CommonTracingParameters
    {
        ...
        DiffuseTracingParameters()
            : numCones(8), autoConeAngle(true), coneAngle(60.0f), ambientColor(0.f), ambientRange(512.0f), ambientScale(1.0f), ambientBias(0.0f), ambientPower(1.0f), ambientDistanceDarkening(0.25f), environmentMap(NULL), environmentMapTint(0.f), enableConeRotation(false), enableRandomConeOffsets(false), normalOffsetFactor(0.5f), tracingSparsity(2), coneNormalGroupingFactor(0.0f), initialOffsetBias(2.0f), initialOffsetDistanceFactor(1.0f), altInitialOffsetBias(2.0f), altInitialOffsetDistanceFactor(1.0f), altNormalOffsetFactor(0.5f), altTracingStep(0.5f), enableTemporalReprojection(false), temporalReprojectionWeight(0.9f), temporalReprojectionMaxDistanceInVoxels(0.25f), temporalReprojectionNormalWeightExponent(20.f), enableReprojectionFromSameFrame(false), enableSparseTracingRefinement(true), interpolationWeightThreshold(1e-4f), enableSSAO(false), SSAO_SurfaceBias(0.2f), SSAO_RadiusWorld(50.f), SSAO_BackgroundViewDepth(1000.f), SSAO_Scale(1.f), SSAO_PowerExponent(2.f)
        {
    ...
    struct SpecularTracingParameters : public CommonTracingParameters
    {
        ...
        SpecularTracingParameters()
            : filter(FILTER_SIMPLE), initialOffsetBias(2.0f), initialOffsetDistanceFactor(1.0f), environmentMap(NULL), environmentMapTint(0.0f), temporalReprojectionWeight(0.8f), tangentJitterScale(0.0f), temporalReprojectionMaxDistanceInVoxels(0.25f), temporalReprojectionNormalWeightExponent(20.f), enableReprojectionFromSameFrame(false)
        {
        }
    };
    ...
    struct IndirectIrradianceMapTracingParameters : public CommonTracingParameters
    {
        ...
        IndirectIrradianceMapTracingParameters()
            : coneAngle(40.f), useAutoNormalization(true), irradianceClampValue(0.f)
        {
        }
    };
    ...
    struct VoxelizationParameters
    {
        ...
        VoxelizationParameters()
            : mapSize(64), allocationMapLodBias(0), stackLevels(5), mipLevels(5), persistentVoxelData(true), simplifiedInvalidate(true), opacityDirectionCount(OpacityDirections::SIX_DIMENSIONAL), enableNvidiaExtensions(true), enableGeometryShaderPassthrough(true), emittanceFormat(EmittanceFormat::PERFORMANCE), emittanceStorageScale(1.0f), useEmittanceInterpolation(false), useHighQualityEmittanceDownsampling(false), enableMultiBounce(false), indirectIrradianceMapLodBias(0)
        {
        }
    ...
    struct TracedSamplesParameters
    {
        ...
        TracedSamplesParameters()
            : onlyContributingSamples(false), coneIndexFilter(0), sampleIndexFilter(0), showConeDirections(false), colorMode(COLOR_MIP_LEVEL)
        {
        }
    };
    ...
    struct MaterialInfo
    {
        ...
        MaterialInfo()
            : pixelShader(NULL), geometryShader(NULL), voxelizationThickness(1.f), opacityNoiseScale(0.f), opacityNoiseBias(0.f), twoSided(false), frontCounterClockwise(false), proportionalEmittance(false), omnidirectionalLight(false), materialSamplingRate(MaterialSamplingRate::FIXED_DEFAULT), enableTriangleCulling(true)
        {
        }

        bool requiresNewState(const MaterialInfo &b) const
        {
            return pixelShader != b.pixelShader ||
    ...
    public:
        struct InputBuffers
        {
            ...
            InputBuffers()
                : gbufferNormalScale(1.0f), gbufferNormalBias(0.0f), gbufferViewport(), gbufferDepth(0), gbufferStencil(0), gbufferNormal(0), gbufferGeoNormal(0), altSettingsStencilMask(0), altSettingsStencilRefValue(1)
            {
            }
        };
    ...
    struct ShaderResources
    {
        ...
        ShaderResources()
            : textureCount(0), samplerCount(0), constantBufferCount(0), unorderedAccessViewCount(0)
        {
    ...
    struct VoxelizationPixelShaderDesc
    {
        ...
        VoxelizationPixelShaderDesc()
            : source(NULL), sourceSize(0), entryFunc("main"), userShaderCodeResources(NULL), useForOpacity(true), useForEmittance(true), canUseDefaultOpacityShader(false), useCoverageSupersampling(false)
        {
        }
    };
    ...
    struct UpdateVoxelizationParameters
    {
        ...
        UpdateVoxelizationParameters()
            : clipmapAnchor(0.f), sceneExtents(Vector3f(FLT_MIN), Vector3f(FLT_MAX)), giRange(512.0f), invalidatedRegions(NULL), invalidatedRegionCount(0), invalidatedLightFrusta(NULL), invalidatedFrustumCount(0)
        {
        }
    };
    ...
    struct DebugRenderParameters
    {
        ...
        DebugRenderParameters()
            : debugMode(DebugRenderMode::DISABLED), destinationTexture(NULL), destinationDepth(NULL), targetOpacity(1.0f), level(0), bitToDisplay(0), voxelsToSkip(0), nearClipZ(0.0f), farClipZ(1.0f)
        {
        }
    };
    ...
    struct ShaderCompilerParameters
    {
        ...
        ShaderCompilerParameters()
            : errorCallback(nullptr), allocator(nullptr), d3dCompilerDLLName(nullptr), multicoreShaderCompilation(true), graphicsAPI(NVRHI::GraphicsAPI::D3D11)
        {
        }
    };
    ...
    struct GIParameters
    {
        ...
        GIParameters()
            : rendererInterface(nullptr), perfMonitor(nullptr), allocator(nullptr), errorCallback(nullptr)
        {
        }
    };

#if VXGI_DYNAMIC_LOAD_LIBRARY
#define VXGI_EXPORT_FUNCTION(rtype, name, ...) \
