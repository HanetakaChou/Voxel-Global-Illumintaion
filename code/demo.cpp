...
#include "../shaders/config.sli"
#include "../shaders/consts.sli"
#include "../shaders/mesh_vertex.sli"
#include "../shaders/voxelization_shader_resource_table.sli"
#include "../shaders/visualization_shader_resource_table.sli"

#include "../dxbc/mesh_voxelization_vertex.inl"
#include "../dxbc/voxelization_pixel.inl"
...
void Demo::Init(ID3D11Device *d3d_device, ID3D11DeviceContext *d3d_device_context, IDXGISwapChain *dxgi_swap_chain)
{
	...
	// Voxelization
	this->m_voxelization_frame_buffer_render_target = NULL;
	{
		D3D11_TEXTURE2D_DESC d3d_texture2d_desc;
		d3d_texture2d_desc.Width = static_cast<uint32_t>(CLIPMAP_SIZE);
		d3d_texture2d_desc.Height = static_cast<uint32_t>(CLIPMAP_SIZE);
		d3d_texture2d_desc.MipLevels = 1U;
		d3d_texture2d_desc.ArraySize = 1U;
		d3d_texture2d_desc.Format = DXGI_FORMAT_R8_UNORM;
		d3d_texture2d_desc.SampleDesc.Count = static_cast<uint32_t>(VOXELIZATION_SAMPLE_COUNT);
		...
		HRESULT res_d3d_device_create_buffer = d3d_device->CreateTexture2D(&d3d_texture2d_desc, NULL, &this->m_voxelization_frame_buffer_render_target);
		assert(SUCCEEDED(res_d3d_device_create_buffer));
	}
	this->m_voxelization_frame_buffer_render_target_view = NULL;
	{
		D3D11_RENDER_TARGET_VIEW_DESC d3d_render_target_view_desc;
		...
		HRESULT res_d3d_device_create_shader_resource_view = d3d_device->CreateRenderTargetView(this->m_voxelization_frame_buffer_render_target, &d3d_render_target_view_desc, &this->m_voxelization_frame_buffer_render_target_view);
	...
	{
		for (uint32_t clipmap_level_index = 0U; clipmap_level_index < static_cast<uint32_t>(CLIPMAP_LEVEL_COUNT); ++clipmap_level_index)
		{
			D3D11_TEXTURE3D_DESC d3d_texture3d_desc;
			d3d_texture3d_desc.Width = static_cast<uint32_t>(CLIPMAP_SIZE);
			d3d_texture3d_desc.Height = static_cast<uint32_t>(CLIPMAP_SIZE);
			d3d_texture3d_desc.Depth = static_cast<uint32_t>(CLIPMAP_SIZE);
	...
	// this->m_clipmap_opacity_unordered_access_view = NULL;
	{
		...
		for (uint32_t clipmap_level_index = 0U; clipmap_level_index < static_cast<uint32_t>(CLIPMAP_LEVEL_COUNT); ++clipmap_level_index)
		{
			...
			d3d_unordered_access_view_desc.Texture3D.WSize = static_cast<uint32_t>(CLIPMAP_SIZE);
	...
	this->m_sponza_vertex_buffer_position = NULL;
	{
		...
		{
#if 0
			mesh_vertex_positions[vertex_index].position_x = DirectX::PackedVector::XMConvertFloatToHalf(sponza_vertex_positions[vertex_index].x);
			mesh_vertex_positions[vertex_index].position_y = DirectX::PackedVector::XMConvertFloatToHalf(sponza_vertex_positions[vertex_index].y);
			mesh_vertex_positions[vertex_index].position_z = DirectX::PackedVector::XMConvertFloatToHalf(sponza_vertex_positions[vertex_index].z);
#else
	...
	this->m_sponza_index_count = sizeof(sponza_indices) / sizeof(sponza_indices[0]);
	{
		D3D11_BUFFER_DESC d3d_buffer_desc;
		d3d_buffer_desc.ByteWidth = sizeof(uint32_t) * this->m_sponza_index_count;
		d3d_buffer_desc.Usage = D3D11_USAGE_DEFAULT;
	...
	// Voxelization
	this->m_mesh_voxelization_vertex_attribute = NULL;
	{
#if 0
		D3D11_INPUT_ELEMENT_DESC d3d_input_elements_desc[] =
		{
			{"LOCATION", 0U, DXGI_FORMAT_R16G16B16A16_FLOAT, 0U, 0U, D3D11_INPUT_PER_VERTEX_DATA, 0U},
		};
#else
		D3D11_INPUT_ELEMENT_DESC d3d_input_elements_desc[] =
			{
				{"LOCATION", 0U, DXGI_FORMAT_R32G32B32A32_FLOAT, 0U, 0U, D3D11_INPUT_PER_VERTEX_DATA, 0U},
			};
#endif
		HRESULT res_d3d_create_input_layout = d3d_device->CreateInputLayout(d3d_input_elements_desc, sizeof(d3d_input_elements_desc) / sizeof(d3d_input_elements_desc[0]), g_dxbc_mesh_voxelization_vertex, sizeof(g_dxbc_mesh_voxelization_vertex), &this->m_mesh_voxelization_vertex_attribute);
		assert(SUCCEEDED(res_d3d_create_input_layout));
	}
	this->m_mesh_voxelization_vertex_shader = NULL;
	{
		HRESULT res_d3d_device_create_vertex_shader = d3d_device->CreateVertexShader(g_dxbc_mesh_voxelization_vertex, sizeof(g_dxbc_mesh_voxelization_vertex), NULL, &this->m_mesh_voxelization_vertex_shader);
		assert(SUCCEEDED(res_d3d_device_create_vertex_shader));
	}
	this->m_voxelization_rasterizer_state = NULL;
	{
		...
		HRESULT res_d3d_device_create_rasterizer_state = d3d_device->CreateRasterizerState(&d3d_rasterizer_desc, &this->m_voxelization_rasterizer_state);
		assert(SUCCEEDED(res_d3d_device_create_rasterizer_state));
	}
	this->m_voxelization_pixel_shader = NULL;
	{
		HRESULT res_d3d_device_create_pixel_shader = d3d_device->CreatePixelShader(g_dxbc_voxelization_pixel, sizeof(g_dxbc_voxelization_pixel), NULL, &this->m_voxelization_pixel_shader);
		assert(SUCCEEDED(res_d3d_device_create_pixel_shader));
	}
	this->m_voxelization_depth_stencil_state = NULL;
	{
		...
		HRESULT res_d3d_device_create_depth_stencil_state = d3d_device->CreateDepthStencilState(&d3d_depth_stencil_desc, &this->m_voxelization_depth_stencil_state);
		assert(SUCCEEDED(res_d3d_device_create_depth_stencil_state));
	}
	this->m_voxelization_constant_buffer_frame = NULL;
	{
		D3D11_BUFFER_DESC d3d_buffer_desc;
		d3d_buffer_desc.ByteWidth = sizeof(voxelization_constant_buffer_frame);
		d3d_buffer_desc.Usage = D3D11_USAGE_DEFAULT;
		...
		HRESULT res_d3d_device_create_buffer = d3d_device->CreateBuffer(&d3d_buffer_desc, NULL, &this->m_voxelization_constant_buffer_frame);
		assert(SUCCEEDED(res_d3d_device_create_buffer));
	}
	this->m_voxelization_constant_buffer_object = NULL;
	{
		D3D11_BUFFER_DESC d3d_buffer_desc;
		d3d_buffer_desc.ByteWidth = sizeof(voxelization_constant_buffer_frame);
		d3d_buffer_desc.Usage = D3D11_USAGE_DEFAULT;
		...
		HRESULT res_d3d_device_create_buffer = d3d_device->CreateBuffer(&d3d_buffer_desc, NULL, &this->m_voxelization_constant_buffer_object);
		assert(SUCCEEDED(res_d3d_device_create_buffer));
	}
	// Visualization
...
void Demo::Tick(ID3D11DeviceContext *d3d_device_context, IDXGISwapChain *dxgi_swap_chain)
{
	...
	struct voxelization_constant_buffer_frame voxelization_b0;
	struct visualization_constant_buffer_frame visualization_b0;
	{
		...
		DirectX::XMVECTOR clipmap_anchor = DirectX::XMVectorAdd(main_camera_position, DirectX::XMVectorScale(DirectX::XMVector3Normalize(main_camera_direction), static_cast<float>(FINEST_VOXEL_SIZE) * static_cast<float>(CLIPMAP_SIZE) * 0.5F));

		// The anchor is snapped to a grid.
		DirectX::XMVECTOR clipmap_center;
		{
			float const coarsest_voxel_size = static_cast<float>(FINEST_VOXEL_SIZE) * static_cast<float>(1U << (static_cast<uint32_t>(CLIPMAP_LEVEL_COUNT) - 1U));
		...
		// Axis
		{
			...
			DirectX::XMFLOAT3 viewport_depth_direction_eye_directions[VIEWPORT_DEPTH_DIRECTION_COUNT] = {
			...
			DirectX::XMFLOAT3 viewport_depth_direction_up_directions[VIEWPORT_DEPTH_DIRECTION_COUNT] = {
			...
			for (uint32_t viewport_depth_direction_index = 0U; viewport_depth_direction_index < VIEWPORT_DEPTH_DIRECTION_COUNT; ++viewport_depth_direction_index)
			{
		...
		for (uint32_t clipmap_level_index = 0U; clipmap_level_index < static_cast<uint32_t>(CLIPMAP_LEVEL_COUNT); ++clipmap_level_index)
		{
			float clipmap_level_voxel_size = static_cast<float>(FINEST_VOXEL_SIZE) * static_cast<float>(1U << clipmap_level_index);

			DirectX::XMMATRIX clipmap_level_projection_matrix = DirectX::XMMatrixOrthographicLH(clipmap_level_voxel_size * static_cast<float>(CLIPMAP_SIZE), clipmap_level_voxel_size * static_cast<float>(CLIPMAP_SIZE), -clipmap_level_voxel_size * static_cast<float>(CLIPMAP_SIZE) * 0.5F, clipmap_level_voxel_size * static_cast<float>(CLIPMAP_SIZE) * 0.5F);
			...
			DirectX::XMVECTOR clipmap_level_toroidal_offset = DirectX::XMVectorMod(DirectX::XMVectorAdd(DirectX::XMVectorMod(DirectX::XMVectorScale(clipmap_center, 1.0F / clipmap_level_voxel_size), DirectX::XMVectorReplicate(static_cast<float>(CLIPMAP_SIZE))), DirectX::XMVectorReplicate(static_cast<float>(CLIPMAP_SIZE))), DirectX::XMVectorReplicate(static_cast<float>(CLIPMAP_SIZE)));
	...
	{
		// Update Frame Constant
		d3d_device_context->UpdateSubresource(this->m_voxelization_constant_buffer_frame, 0U, NULL, &voxelization_b0, sizeof(struct voxelization_constant_buffer_frame), sizeof(struct voxelization_constant_buffer_frame));

		// Frame Buffer
		{
			D3D11_VIEWPORT viewports[1] = {{0.0F, 0.0F, static_cast<float>(CLIPMAP_SIZE), static_cast<float>(CLIPMAP_SIZE), 0.0F, 1.0F}};
			...
			ID3D11RenderTargetView *const render_target_views[1U] = {this->m_voxelization_frame_buffer_render_target_view};

			ID3D11UnorderedAccessView *unordered_access_views[CLIPMAP_LEVEL_COUNT];
			for (uint32_t clipmap_level_index = 0U; clipmap_level_index < static_cast<uint32_t>(CLIPMAP_LEVEL_COUNT); ++clipmap_level_index)
			{
				unordered_access_views[clipmap_level_index] = this->m_clipmap_opacity_unordered_access_view[clipmap_level_index];
			}
			d3d_device_context->OMSetRenderTargetsAndUnorderedAccessViews(1U, render_target_views, NULL, 1U, CLIPMAP_LEVEL_COUNT, unordered_access_views, NULL);
		...
		// Pipeline State Object
		{
			d3d_device_context->IASetInputLayout(this->m_mesh_voxelization_vertex_attribute);
			d3d_device_context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
			d3d_device_context->VSSetShader(this->m_mesh_voxelization_vertex_shader, NULL, 0U);
			d3d_device_context->RSSetState(this->m_voxelization_rasterizer_state);
			d3d_device_context->PSSetShader(this->m_voxelization_pixel_shader, NULL, 0U);
			d3d_device_context->OMSetDepthStencilState(this->m_voxelization_depth_stencil_state, 0U);
			d3d_device_context->VSSetConstantBuffers(0U, 1U, &this->m_voxelization_constant_buffer_frame);
			d3d_device_context->VSSetConstantBuffers(1U, 1U, &this->m_voxelization_constant_buffer_object);
			d3d_device_context->PSSetConstantBuffers(0U, 1U, &this->m_voxelization_constant_buffer_frame);
		}

		// Update Object Constant
		{
			struct voxelization_constant_buffer_object voxelization_b1;
			...
			d3d_device_context->UpdateSubresource(this->m_voxelization_constant_buffer_object, 0U, NULL, &voxelization_b1, sizeof(struct voxelization_constant_buffer_object), sizeof(struct voxelization_constant_buffer_object));
		}

		// Draw Sponza
		{
			ID3D11Buffer *vertex_buffers[1] = {this->m_sponza_vertex_buffer_position};
			UINT strides[1] = {sizeof(mesh_vertex_position)};
			UINT offsets[1] = {0U};
			d3d_device_context->IASetVertexBuffers(0U, 1U, vertex_buffers, strides, offsets);
			d3d_device_context->IASetIndexBuffer(this->m_sponza_index_buffer, DXGI_FORMAT_R32_UINT, 0U);
			d3d_device_context->DrawIndexedInstanced(this->m_sponza_index_count, static_cast<uint32_t>(VIEWPORT_DEPTH_DIRECTION_COUNT) * static_cast<uint32_t>(CLIPMAP_LEVEL_COUNT), 0U, 0U, 0U);
		}
	}
	...
	// Visualization Pass
	{
		...
		// Pipeline State Object
		{
			...
			ID3D11ShaderResourceView* shader_resource_views[CLIPMAP_LEVEL_COUNT];
			for (uint32_t clipmap_level_index = 0U; clipmap_level_index < static_cast<uint32_t>(CLIPMAP_LEVEL_COUNT); ++clipmap_level_index)
			{
				shader_resource_views[clipmap_level_index] = this->m_clipmap_opacity_shader_resource_view[clipmap_level_index];
			}
			d3d_device_context->PSSetShaderResources(0U, static_cast<uint32_t>(CLIPMAP_LEVEL_COUNT), shader_resource_views);
		}
