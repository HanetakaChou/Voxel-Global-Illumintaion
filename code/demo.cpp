...
void Demo::Init(ID3D11Device *d3d_device, ID3D11DeviceContext *d3d_device_context, IDXGISwapChain *dxgi_swap_chain)
{
	...
	// Light Injection
	this->m_vct_light_injection_mesh_vertex_attribute = NULL;
	{
#if 0
...
#endif
		HRESULT res_d3d_create_input_layout = d3d_device->CreateInputLayout(d3d_input_elements_desc, sizeof(d3d_input_elements_desc) / sizeof(d3d_input_elements_desc[0]), g_dxbc_vct_light_injection_mesh_vertex, sizeof(g_dxbc_vct_light_injection_mesh_vertex), &this->m_vct_light_injection_mesh_vertex_attribute);
	...
	this->m_vct_light_injection_constant_buffer_frame = NULL;
	{
		D3D11_BUFFER_DESC d3d_buffer_desc;
		...
		HRESULT res_d3d_device_create_buffer = d3d_device->CreateBuffer(&d3d_buffer_desc, NULL, &this->m_vct_light_injection_constant_buffer_frame);
		assert(SUCCEEDED(res_d3d_device_create_buffer));
	}
	this->m_vct_light_injection_constant_buffer_object = NULL;
	{
		D3D11_BUFFER_DESC d3d_buffer_desc;
		d3d_buffer_desc.ByteWidth = sizeof(light_injection_per_batch_constant_buffer);
		d3d_buffer_desc.Usage = D3D11_USAGE_DEFAULT;
		...
		HRESULT res_d3d_device_create_buffer = d3d_device->CreateBuffer(&d3d_buffer_desc, NULL, &this->m_vct_light_injection_constant_buffer_object);
		assert(SUCCEEDED(res_d3d_device_create_buffer));
	}
	// Visualization
...
void Demo::Tick(ID3D11DeviceContext *d3d_device_context, IDXGISwapChain *dxgi_swap_chain)
{
	...
	{
		// Update Frame Constant
		d3d_device_context->UpdateSubresource(this->m_vct_light_injection_constant_buffer_frame, 0U, NULL, &voxelization_b0, sizeof(struct light_injection_per_batch_constant_buffer), sizeof(struct light_injection_per_batch_constant_buffer));
		...
		// Pipeline State Object
		{
			d3d_device_context->IASetInputLayout(this->m_vct_light_injection_mesh_vertex_attribute);
			...
			d3d_device_context->VSSetConstantBuffers(0U, 1U, &this->m_vct_light_injection_constant_buffer_frame);
			d3d_device_context->VSSetConstantBuffers(1U, 1U, &this->m_vct_light_injection_constant_buffer_object);
			d3d_device_context->PSSetConstantBuffers(0U, 1U, &this->m_vct_light_injection_constant_buffer_frame);
		}
		...
		// Update Object Constant
		{
			...
			d3d_device_context->UpdateSubresource(this->m_vct_light_injection_constant_buffer_object, 0U, NULL, &voxelization_b1, sizeof(struct light_injection_per_draw_constant_buffer), sizeof(struct light_injection_per_draw_constant_buffer));
		}
		...
		// Draw Sponza
		{
			...
			d3d_device_context->DrawIndexedInstanced(this->m_sponza_index_count, static_cast<uint32_t>(VIEWPORT_DEPTH_DIRECTION_COUNT) * static_cast<uint32_t>(VCT_CLIPMAP_LEVEL_COUNT), 0U, 0U, 0U);
		}
	}
