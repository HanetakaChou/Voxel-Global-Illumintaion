...
DEFINE_APPLICATION_MAIN(vct_app)

bool vct_app::Init()
{
	{
		setBorderless(this->pWindow, 1280U, 720U);
	...
	this->m_graphics_queue = NULL;
	{
		...
		addQueue(this->m_renderer, &queue_desc, &this->m_graphics_queue);
	}
	...
	this->m_upload_ring_buffer_offset_alignment = this->m_renderer->pGpu->mSettings.mUniformBufferAlignment;
	...
	for (uint32_t frame_throttling_index = 0U; frame_throttling_index < FRAME_THROTTLING_COUNT; ++frame_throttling_index)
	{
		this->m_cmd_pools[frame_throttling_index] = NULL;
		{
			...
			addCmdPool(this->m_renderer, &cmd_pool_desc, &this->m_cmd_pools[frame_throttling_index]);
		}
		...
		this->m_cmds[frame_throttling_index] = NULL;
		{
			...
			addCmd(this->m_renderer, &cmd_desc, &this->m_cmds[frame_throttling_index]);
		}
		assert(this->m_cmds[frame_throttling_index]);

		addSemaphore(this->m_renderer, &this->m_acquire_next_image_semaphore[frame_throttling_index]);

		addSemaphore(this->m_renderer, &this->m_queue_submit_semaphore[frame_throttling_index]);

		addFence(this->m_renderer, &this->m_fences[frame_throttling_index]);
...
void vct_app::Exit()
{
	...
	for (uint32_t frame_throttling_index = 0U; frame_throttling_index < FRAME_THROTTLING_COUNT; ++frame_throttling_index)
	{
		...
		removeFence(this->m_renderer, this->m_fences[frame_throttling_index]);
		this->m_fences[frame_throttling_index] = NULL;

		assert(this->m_queue_submit_semaphore[frame_throttling_index]);
		removeSemaphore(this->m_renderer, this->m_queue_submit_semaphore[frame_throttling_index]);
		...
		removeSemaphore(this->m_renderer, this->m_acquire_next_image_semaphore[frame_throttling_index]);
		this->m_acquire_next_image_semaphore[frame_throttling_index] = NULL;

		assert(this->m_cmds[frame_throttling_index]);
		removeCmd(this->m_renderer, this->m_cmds[frame_throttling_index]);
		this->m_cmds[frame_throttling_index] = NULL;

		assert(this->m_cmd_pools[frame_throttling_index]);
		removeCmdPool(this->m_renderer, this->m_cmd_pools[frame_throttling_index]);
		this->m_cmd_pools[frame_throttling_index] = NULL;
	}
	...
	assert(this->m_graphics_queue);
	removeQueue(this->m_renderer, this->m_graphics_queue);
	this->m_graphics_queue = NULL;
...
bool vct_app::Load(ReloadDesc *pReloadDesc)
{
	...
	{
		fsSetPathForResourceDir(pSystemFileIO, RM_CONTENT, RD_SHADER_BINARIES, "shaders");

		this->m_light_injection_shader = NULL;
		{
			ShaderLoadDesc light_injection_shader_load_desc = {};
			light_injection_shader_load_desc.mStages[0].pFileName = "light_injection.vert";
			light_injection_shader_load_desc.mStages[1].pFileName = "light_injection.frag";
...
void vct_app::Unload(ReloadDesc *pReloadDesc)
{
...
void vct_app::Update(float deltaTime)
{
	// Camera
	{
