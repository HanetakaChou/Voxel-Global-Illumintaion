...
RWTexture2D<float4> u_CoarseOutput1 : REGISTER(u, 1);

RWTexture2D<float4> u_CoarseOutput2 : REGISTER(u, 2);
...
[numthreads(8, 8, 1)] void main(in uint3 gfsdk_GroupIdx : SV_GroupID, in uint3 gfsdk_GroupThreadIdx : SV_GroupThreadID, in uint3 gfsdk_GlobalIdx : SV_DispatchThreadID)
{
    ...
    u_CoarseOutput1[coarsePixelPos] = float4(0.0, 0.0, 0.0, 0.0);

    u_CoarseOutput2[coarsePixelPos] = float4(0.0, 0.0, 0.0, 0.0);
}
